...
def wu_loss(r_outputs, c_outputs, r_labels, x_input):
    """
    Weighted Unity Loss (used in baseline model).
    Returns vector of L1 losses per regression output (length 9).
    """
    # Slice only first 9 outputs to match r_labels
    r_outputs_trimmed = r_outputs[:, :9]

    # Element-wise absolute error
    abs_error = torch.abs(r_outputs_trimmed - r_labels)  # shape: [batch_size, 9]

    # Mean per regression output → vector of shape [9]
    loss_vector = abs_error.mean(dim=0)
